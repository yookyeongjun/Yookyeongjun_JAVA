...
//Thread 클래스를 상속 받고 ActionListener 인터페이스를 구현하는 Network_ex7_Chat_Server 클래스를 정의한다. 
	...
	public void actionPerformed(ActionEvent action){
		//텍스트 필드에 대한 이벤트 처리를 위해 actionPerformed() 메소드를 구현한다.
	...
	public void run() {
		try {
			...
			//stop 변수가 true 가 될 때까지 클라이언트에서 전송된 데이터를 입력받는다.
			dis.close();
			s1.close();
